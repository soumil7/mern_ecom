...
import './PoojaProducts.css'; // Create a separate CSS file for Pooja Products if needed
import { FaRegBookmark, FaBookmark } from 'react-icons/fa'; // Font Awesome icons for bookmark
import FilterButton from '../../Components/FilterButton/FilterButton';
import { pooja } from '../../assets/assets';

const PoojaProducts = ({ cart, setCart, bookmarkedProducts, setBookmarkedProducts, url, recentlyViewed, setRecentlyViewed }) => {
  const [products, setProducts] = useState([]);

  // Fetch Pooja products using Axios
  useEffect(() => {
  ...
  return (
    <div className="pooja-container">
      <FilterButton />
      <h2>Pooja Products</h2>
      <div className="product-grid">
        {Array.isArray(products) && products
          .filter(product => product.product === 'Pooja Products') // Filter by category
          .map((product) => (
            <div 
